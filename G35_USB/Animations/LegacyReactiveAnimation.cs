...
namespace G35_USB
{
	public class LegacyReactiveAnimation:G35_USB.AbstractReactiveAnimation
	{
		#region Definitions

		private List<LED> G35_Lights_Unprocessed = new List<LED> ();
...
//			}
		...
		private void AudioMeter_Extended_Rainbow (bool UseSolidColors)
		{
			...
			if (UseSolidColors) {
				G35_Lights_Unprocessed [LightSystem.LIGHT_INDEX_MIDDLE].SetColor (VU_RandomColor);
				G35_Lights_Unprocessed [LightSystem.LIGHT_INDEX_MIDDLE - 1].SetColor (VU_RandomColor);
			} else {
				G35_Lights_Unprocessed [LightSystem.LIGHT_INDEX_MIDDLE].R = ColorShift_Red;
				G35_Lights_Unprocessed [LightSystem.LIGHT_INDEX_MIDDLE].G = ColorShift_Green;
				G35_Lights_Unprocessed [LightSystem.LIGHT_INDEX_MIDDLE].B = ColorShift_Blue;
				G35_Lights_Unprocessed [LightSystem.LIGHT_INDEX_MIDDLE - 1].R = ColorShift_Red;
				G35_Lights_Unprocessed [LightSystem.LIGHT_INDEX_MIDDLE - 1].G = ColorShift_Green;
				G35_Lights_Unprocessed [LightSystem.LIGHT_INDEX_MIDDLE - 1].B = ColorShift_Blue;
			}

			if (UseSolidColors) {
				LightProcessing.ShiftLightsOutward (G35_Lights_Unprocessed, (int)VU_ShiftSpeed);
			} else {
				LightProcessing.ShiftLightsOutward (G35_Lights_Unprocessed, 1);
			}

			for (int i = 0; i < LightSystem.LIGHT_COUNT; i++) {
				CurrentFrame [i].R = (byte)Math.Min (G35_Lights_Unprocessed [i].R + ColorShift_HighBoost, 255);
				CurrentFrame [i].G = G35_Lights_Unprocessed [i].G;
				CurrentFrame [i].B = (byte)Math.Min (G35_Lights_Unprocessed [i].B + ColorShift_LowBoost, 255);
			}
		...
		private void AudioMeter_Extended_RainbowBeat (bool BeatBasedBrightness)
		{
			if (BeatBasedBrightness) {
				G35_Lights_Unprocessed [LightSystem.LIGHT_INDEX_MIDDLE].Brightness = (byte)MathUtilities.ConvertRange (Audio_Low_Intensity, 0, 1, LightSystem.Color_MID, 255);
				G35_Lights_Unprocessed [LightSystem.LIGHT_INDEX_MIDDLE - 1].Brightness = (byte)MathUtilities.ConvertRange (Audio_Low_Intensity, 0, 1, LightSystem.Color_MID, 255);

				LightProcessing.ShiftLightsBrightnessOutward (G35_Lights_Unprocessed, 1);
			} else {
				for (int i = 0; i < LightSystem.LIGHT_COUNT; i++) {
		...
		private void AudioMeter_Extended_Hueshift_Beat ()
		{
			...
			G35_Lights_Unprocessed [LightSystem.LIGHT_INDEX_MIDDLE].Brightness = (byte)MathUtilities.ConvertRange (Audio_Low_Intensity, 0, 1, LightSystem.Color_DARK, 255);
			G35_Lights_Unprocessed [LightSystem.LIGHT_INDEX_MIDDLE - 1].Brightness = G35_Lights_Unprocessed [LightSystem.LIGHT_INDEX_MIDDLE].Brightness;

			LightProcessing.ShiftLightsBrightnessOutward (G35_Lights_Unprocessed, 2);
			...
			if (VU_Hueshift_Beat_Pause_Fading_Off_Count >= MathUtilities.ConvertRange (Math.Max(Audio_Mid_Intensity - VU_Hueshift_Beat_Pause_Fading_Intensity_Floor, 0), 0, 1 - VU_Hueshift_Beat_Pause_Fading_Intensity_Floor, VU_Hueshift_Beat_Pause_Fading_Min_Delay, VU_Hueshift_Beat_Pause_Fading_Max_Delay)) {
				...
				for (int i = 0; i < LightSystem.LIGHT_COUNT; i++) {
					G35_Lights_Unprocessed [i].R = ColorShift_Red;
					G35_Lights_Unprocessed [i].G = ColorShift_Green;
					G35_Lights_Unprocessed [i].B = ColorShift_Blue;
				}
			} else {
			...
			for (int i = 0; i < (LightSystem.LIGHT_COUNT / 2); i++) {
				CurrentFrame [i].R = G35_Lights_Unprocessed [i].R;
				CurrentFrame [i].G = G35_Lights_Unprocessed [i].G;
				CurrentFrame [i].B = G35_Lights_Unprocessed [i].B;
				CurrentFrame [i].Brightness = G35_Lights_Unprocessed [i].Brightness;
			}
