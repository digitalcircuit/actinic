	...
	public class ReactiveSystem
	{
		...
		public static void PrintAudioInformationToConsole (AbstractReactiveAnimation CurrentAnimation)
		{
			...
			if (Processing_Show_Analysis) {
				...
				Console.WriteLine ("Intensity: " + Math.Round (CurrentAnimation.Audio_Realtime_Intensity, 3).ToString ().PadRight (5) + " (avg: " + Math.Round (CurrentAnimation.Audio_Average_Intensity, 3).ToString ().PadRight (5) + ", delta: " + VU_Intensity_Delta_Sign + Math.Abs (Math.Round (CurrentAnimation.Audio_Delta_Intensity, 3)).ToString ().PadRight (5) + ")  Smoothing: " + Math.Round (CurrentAnimation.SmoothingConstant, 3).ToString ().PadRight (5) + "  Color fade: " + CurrentAnimation.ColorShift_Amount.ToString ().PadRight (4));
